...
<FocusBox name="button" dimensions="width:45; height:45; center-y:50%; center-x:50%;" class="ui_button">
	...
	<WebImage name="Icon" dimensions="dock:fill" style="fixed-bounds:true; valign:center; halign:center; shadow:1.0; cursor:sys_hand;"/>
		...
		local ING_HOST = System.GetOperatorSetting("ingame_host");
		local icon_url = ING_HOST.."/assets/items/64/"..listing.icon..".png";
...
Table.CreateCellBlueprint("quality", [[
	<Text name="quality" dimensions="left:5; height:100%; width:100%-5;" class="LeftText" style="font:Demi_10;"/>
...
Table.CreateCellBlueprint("number", [[
	...
	TXT:SetText(tostring(round(tonumber(num),2)));
end);
...
Table.CreateCellBlueprint("price_per_unit", [[
	...
	if(listing.quantity > 1) then
		TXT:SetText(""..listing.price_per_unit.." cy/u");
	end
end);
...
Table.CreateCellBlueprint("buy_btn", [[
	...
	if(SELLPAGE.IsListingMine(data.id)) then
		if(data.purchased) then
			...
			CHOICE:AddItem("Claim", {action="reap", WIDGET=CHOICE, listing=data});
			if(SELLPAGE.FindItem(data)) then
				CHOICE:AddItem("List more...", {action="reap_list_again", WIDGET=CHOICE, listing=data});
			end
		else
			
			local unixTime = timeParser(data.expires_at);
			local deltaSeconds = System.GetElapsedUnixTime(unixTime);

			if(deltaSeconds > 0) then
				CHOICE:TintPlate("#FF0000");
				CHOICE:AddItem("Expired", "nothing");
				CHOICE:AddItem("Remove", {action="cancel", WIDGET=CHOICE, listing=data});
			else
				CHOICE:AddItem("Action", "nothing");
				CHOICE:AddItem("Cancel", {action="cancel", WIDGET=CHOICE, listing=data});
			end

			if(SELLPAGE.FindItem(data)) then
				CHOICE:AddItem("List more...", {action="list_more", WIDGET=CHOICE, listing=data});
			end
			CHOICE:AddItem("Relist", {action="relist", WIDGET=CHOICE, listing=data});
		end
	else
		if(data.price_cy <= Player.GetItemCount(10)) then
			CHOICE:TintPlate("#00FF00");
			CHOICE:AddItem("Action", "nothing");
			CHOICE:AddItem("Buy", {action="buy_item", WIDGET=CHOICE, listing=data});
			CHOICE:AddItem("Relist", {action="buy_item", WIDGET=CHOICE, listing=data});
			if(SELLPAGE.FindItem(data)) then
				CHOICE:AddItem("List...", {action="list_more", WIDGET=CHOICE, listing=data});
			end
		else
			CHOICE:TintPlate("#FF0000");
...
Table.CreateCellBlueprint("expires_at", [[
	...
	local TXT = WIDGET:GetChild("number");
	local unixTime = timeParser(date);
	local text = timeCountDown(unixTime);
	local deltaSeconds = System.GetElapsedUnixTime(unixTime);
	if(deltaSeconds > 0) then
